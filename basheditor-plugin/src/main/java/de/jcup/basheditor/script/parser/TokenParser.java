...
import static de.jcup.basheditor.script.parser.ParserState.INSIDE_IF;
...
public class TokenParser {
    ...
    private boolean handleString(ParserState stringState, ParseContext context, ParserState... otherStringStates) {
        ...
        boolean singleQuoteState = ParserState.INSIDE_SINGLE_QUOTE.equals(stringState);
        boolean charBeforeEscapeSign = context.isCharBeforeEscapeSign();
        if (!singleQuoteState && charBeforeEscapeSign) {
            /* escaped */
            context.appendCharToText();
            return true;
        }
        if (context.inState(stringState)) {
            ...
            if (charBeforeEscapeSign) {
                /*
                 * okay, we are NOT in single quote mode, but have \ before, so this is NOT
                 * single quote mode enter but only an esacped char ! so we just return false
                 * here so its handled as a normal character
                 */
                return false;
            }
        }
