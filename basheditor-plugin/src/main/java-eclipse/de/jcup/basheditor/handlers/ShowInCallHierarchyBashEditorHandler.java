...
public class ShowInCallHierarchyBashEditorHandler extends AbstractBashEditorHandler {
    ...
    protected void executeOnBashEditor(BashEditor bashEditor) {
        ...
        if (selection instanceof ITextSelection) {
            ...
            if (item != null) {
                entry = new BashCallHierarchyEntry().setElement(item).setOffset(item.getOffset()).setLength(item.getLength());
            } else {
                String text = textSelection.getText();
                if (text == null || text.length() == 0) {
                    /* okay, complete file/resource */
                    entry = new BashCallHierarchyEntry().setElement(bashEditor.resolveResource()).setOffset(offset).setLength(0);
                } else {
                    entry = new BashCallHierarchyEntry().setElement(bashEditor.resolveResource()).setOffset(offset).setLength(text.length());
                }
            }
        }
        if (entry != null) {
