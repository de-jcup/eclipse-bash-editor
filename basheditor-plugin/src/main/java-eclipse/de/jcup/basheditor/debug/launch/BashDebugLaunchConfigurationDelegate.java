...
public class BashDebugLaunchConfigurationDelegate extends LaunchConfigurationDelegate {
    ...
    public void launch(ILaunchConfiguration configuration, String mode, ILaunch launch, IProgressMonitor monitor) throws CoreException {
        ...
        if (!debug && !runOnly) {
            throw new IllegalStateException("Only run and debug mode supported, not :" + mode);
        }
        ...
        boolean stopOnStartup = debug && configuration.getAttribute(BashDebugConstants.LAUNCH_ATTR_STOP_ON_STARTUP, false);
        ...
        int port = debug ? configuration.getAttribute(BashDebugConstants.LAUNCH_ATTR_SOCKET_PORT, BashDebugConstants.DEFAULT_DEBUG_PORT) : -1;
        ...
        if (canDoAutoRun) {
            ...
            Map<String, String> environment = configuration.getAttribute(LAUNCH_ENVIRONMENT_PROPERTIES, Collections.emptyMap());
            ...
            Process process = terminalLauncher.execute(programFile, params, getPreferences().getTerminalCommand(), getPreferences().getStarterCommand(), environment, port);
    ...
    private void showDebugView() {
        EclipseUtil.safeAsyncExec(() -> {
            ...
            if (page != null) {
                try {
                    page.showView("org.eclipse.debug.ui.DebugView");
                } catch (Exception e) {
                    BashEditorUtil.logError("Was not able to show debug view", e);
                }
            }
        });
    }
     ...
     */
    private IDebugTarget createDebugTargetOrNull(ILaunch launch, Process terminalProcess, boolean debug, IFile programFileResource, int port, TerminalLauncher terminalLauncher)
     ...
     * @throws DebugException
     */
    private boolean canAccessBashScript(ILaunch launch, String program, IFile programFileResource) throws DebugException {
